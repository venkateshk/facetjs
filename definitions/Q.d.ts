...
declare module Q {
    ...
    when: {
        ...
        <T>(value: T | IPromise<T>): Promise<T>;
    }
     ...
     */
    // try(method: Function, ...args: any[]): Promise<any>;

    fbind: <T>(method: (...args: any[]) => T | IPromise<T>, ...args: any[]) => PromiseGen<T>;
    ...
    denodeify: <T>(nodeFunction: Function, ...args: any[]) => PromiseGen<T>;
    nbind: <T>(nodeFunction: Function, thisArg: any, ...args: any[]) => PromiseGen<T>;
    nfbind: <T>(nodeFunction: Function, ...args: any[]) => PromiseGen<T>;
     ...
     */
    all: <T>(promises: IPromise<T>[]) => Promise<T[]>;

    /**
     * Returns a promise that is fulfilled with an array of promise state snapshots, but only after all the original promises have settled, i.e. become either fulfilled or rejected.
     */
    allSettled: <T>(promises: IPromise<T>[]) => Promise<PromiseState<T>[]>;

    allResolved: <T>(promises: IPromise<T>[]) => Promise<Promise<T>[]>;

    /**
     * Like then, but "spreads" the array into a variadic fulfillment handler. If any of the promises in the array are rejected, instead calls onRejected with the first rejected promise's rejection reason.
     * This is especially useful in conjunction with all.
     */
    spread: <T, U>(promises: IPromise<T>[], onFulfilled: (...args: T[]) => U | IPromise<U>, onRejected?: (reason: any) => U | IPromise<U>) => Promise<U>;
    ...
    Promise: <T>(resolver: (resolve: (val: T | IPromise<T>) => void , reject: (reason: any) => void , notify: (progress: any) => void ) => void ) => Promise<T>;
     ...
     */
    promised: <T>(callback: (...args: any[]) => T) => PromiseGen<T>;
     ...
     */
    async: (generatorFunction: any) => PromiseGen<any>;
    ...
    resolve: {
         ...
         * Calling resolve with a non-promise value causes promise to be fulfilled with that value.
         */
        <T>(object: T): Promise<T>;
    }

    }
}

declare var Q: Q.Base;
