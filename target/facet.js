...
(function() {
  var FacetJob, Segment, arraySubclass, divideLength, facet, flatten, getProp, stripeTile,
    __slice = [].slice;
  ...
  facet.plot = {
    rect: function(_arg) {
      ...
      return function(segment) {
        ...
        segment.node.append('rect').datum(segment).attr('width', stage.width).attr('height', stage.height).style('fill', fill).style('stroke', stroke);
      };
    },
    text: function(_arg) {
      var anchor, baseline, color, text;
      color = _arg.color, text = _arg.text, anchor = _arg.anchor, baseline = _arg.baseline;
      return function(segment) {
        var node, stage;
        stage = segment.getStage();
        if (stage.type !== 'point') {
        ...
        node = segment.node.append('text').datum(segment).attr('x', stage.x).attr('y', stage.y);
