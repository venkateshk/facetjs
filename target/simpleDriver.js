// Generated by CoffeeScript 1.3.1
(function() {
  var applyFns, async, compareFns, computeQuery, driverUtil, exports, filterFns, makeApplyFn, makeFilterFn, makeSplitFn, rq, sortFns, splitFns,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  rq = function(module) {
    var moduleParts;
    if (typeof window === 'undefined') {
      return require(module);
    } else {
      moduleParts = module.split('/');
      return window[moduleParts[moduleParts.length - 1]];
    }
  };

  async = rq('async');

  driverUtil = rq('./driverUtil');

  if (typeof exports === 'undefined') {
    exports = {};
  }

  filterFns = {
    is: function(_arg) {
      var attribute, value;
      attribute = _arg.attribute, value = _arg.value;
      return function(d) {
        return d[attribute] === value;
      };
    },
    "in": function(_arg) {
      var attribute, values;
      attribute = _arg.attribute, values = _arg.values;
      return function(d) {
        var _ref;
        return _ref = d[attribute], __indexOf.call(values, _ref) >= 0;
      };
    },
    match: function(_arg) {
      var attribute, expression;
      attribute = _arg.attribute, expression = _arg.expression;
      expression = new RegExp(expression);
      return function(d) {
        return expression.test(d[attribute]);
      };
    },
    within: function(_arg) {
      var attribute, range;
      attribute = _arg.attribute, range = _arg.range;
      if (!(Array.isArray(range) && range.length === 2)) {
        throw new TypeError("range must be an array of two things");
      }
      return function(d) {
        var _ref;
        return (range[0] <= (_ref = d.attribute) && _ref < range[1]);
      };
    },
    not: function(_arg) {
      var filter;
      filter = _arg.filter;
      if (typeof filter !== 'object') {
        throw new TypeError("filter must be a filter object");
      }
      filter = makeFilterFn(filter);
      return function(d) {
        return !filter(d);
      };
    },
    and: function(_arg) {
      var filters;
      filters = _arg.filters;
      if (!filters.length) {
        throw new TypeError('must have some filters');
      }
      filters = filters.map(makeFilterFn);
      return function(d) {
        var filter, _i, _len;
        for (_i = 0, _len = filters.length; _i < _len; _i++) {
          filter = filters[_i];
          if (!filter(d)) {
            return false;
          }
        }
        return true;
      };
    },
    or: function(_arg) {
      var filters;
      filters = _arg.filters;
      if (!filters.length) {
        throw new TypeError('must have some filters');
      }
      filters = filters.map(makeFilterFn);
      return function(d) {
        var filter, _i, _len;
        for (_i = 0, _len = filters.length; _i < _len; _i++) {
          filter = filters[_i];
          if (filter(d)) {
            return true;
          }
        }
        return false;
      };
    }
  };

  makeFilterFn = function(filter) {
    var filterFn;
    filterFn = filterFns[filter.type];
    if (!filterFn) {
      throw new Error("filter type '" + filter.type + "' not defined");
    }
    return filterFn(filter);
  };

  splitFns = {
    identity: function(_arg) {
      var attribute;
      attribute = _arg.attribute;
      if (typeof attribute !== 'string') {
        throw new Error('attribute not defined');
      }
      return function(d) {
        var _ref;
        return (_ref = d[attribute]) != null ? _ref : null;
      };
    },
    continuous: function(_arg) {
      var attribute, offset, size;
      attribute = _arg.attribute, size = _arg.size, offset = _arg.offset;
      if (typeof attribute !== 'string') {
        throw new Error('attribute not defined');
      }
      if (!(size > 0)) {
        throw new Error("size has to be positive (is: " + size + ")");
      }
      return function(d) {
        var b, num;
        num = Number(d[attribute]);
        if (isNaN(num)) {
          return null;
        }
        b = Math.floor((num + offset) / size) * size - offset;
        return [b, b + size];
      };
    },
    time: function(_arg) {
      var attribute, duration, timezone;
      attribute = _arg.attribute, duration = _arg.duration, timezone = _arg.timezone;
      if (typeof attribute !== 'string') {
        throw new Error('attribute not defined');
      }
      switch (duration) {
        case 'second':
          return function(d) {
            var de, ds;
            ds = new Date(d[attribute]);
            if (isNaN(ds)) {
              return null;
            }
            ds.setUTCMilliseconds(0);
            de = new Date(ds);
            de.setUTCMilliseconds(1000);
            return [ds, de];
          };
        case 'minute':
          return function(d) {
            var de, ds;
            ds = new Date(d[attribute]);
            if (isNaN(ds)) {
              return null;
            }
            ds.setUTCSeconds(0, 0);
            de = new Date(ds);
            de.setUTCSeconds(60);
            return [ds, de];
          };
        case 'hour':
          return function(d) {
            var de, ds;
            ds = new Date(d[attribute]);
            if (isNaN(ds)) {
              return null;
            }
            ds.setUTCMinutes(0, 0, 0);
            de = new Date(ds);
            de.setUTCMinutes(60);
            return [ds, de];
          };
        case 'day':
          return function(d) {
            var de, ds;
            ds = new Date(d[attribute]);
            if (isNaN(ds)) {
              return null;
            }
            ds.setUTCHours(0, 0, 0, 0);
            de = new Date(ds);
            de.setUTCHours(24);
            return [ds, de];
          };
      }
    }
  };

  makeSplitFn = function(cmd) {
    var splitFn;
    splitFn = splitFns[cmd.bucket];
    if (!splitFn) {
      throw new Error("No such bucket `" + cmd.bucket + "` in split");
    }
    return splitFn(cmd);
  };

  applyFns = {
    constant: function(_arg) {
      var value;
      value = _arg.value;
      return function() {
        return value;
      };
    },
    count: function() {
      return function(ds) {
        return ds.length;
      };
    },
    sum: function(_arg) {
      var attribute;
      attribute = _arg.attribute;
      return function(ds) {
        var d, sum, _i, _len;
        sum = 0;
        for (_i = 0, _len = ds.length; _i < _len; _i++) {
          d = ds[_i];
          sum += Number(d[attribute]);
        }
        return sum;
      };
    },
    average: function(_arg) {
      var attribute;
      attribute = _arg.attribute;
      return function(ds) {
        var d, sum, _i, _len;
        sum = 0;
        for (_i = 0, _len = ds.length; _i < _len; _i++) {
          d = ds[_i];
          sum += Number(d[attribute]);
        }
        return sum / ds.length;
      };
    },
    min: function(_arg) {
      var attribute;
      attribute = _arg.attribute;
      return function(ds) {
        var d, min, _i, _len;
        min = +Infinity;
        for (_i = 0, _len = ds.length; _i < _len; _i++) {
          d = ds[_i];
          min = Math.min(min, Number(d[attribute]));
        }
        return min;
      };
    },
    max: function(_arg) {
      var attribute;
      attribute = _arg.attribute;
      return function(ds) {
        var d, max, _i, _len;
        max = -Infinity;
        for (_i = 0, _len = ds.length; _i < _len; _i++) {
          d = ds[_i];
          max = Math.max(max, Number(d[attribute]));
        }
        return max;
      };
    },
    uniqueCount: function(_arg) {
      var attribute;
      attribute = _arg.attribute;
      return function(ds) {
        var count, d, seen, v, _i, _len;
        seen = {};
        count = 0;
        for (_i = 0, _len = ds.length; _i < _len; _i++) {
          d = ds[_i];
          v = d[attribute];
          if (!seen[v]) {
            count++;
            seen[v] = 1;
          }
        }
        return count;
      };
    },
    quantile: function(_arg) {
      var attribute, quantile;
      attribute = _arg.attribute, quantile = _arg.quantile;
      return function(ds) {
        throw "not implemented yet (ToDo)";
      };
    },
    add: function(_arg) {
      var operands;
      operands = _arg.operands;
      operands = operands.map(makeApplyFn);
      return function(ds) {
        return operands[0](ds) + operands[1](ds);
      };
    },
    subtract: function(_arg) {
      var operands;
      operands = _arg.operands;
      operands = operands.map(makeApplyFn);
      return function(ds) {
        return operands[0](ds) - operands[1](ds);
      };
    },
    multiply: function(_arg) {
      var operands;
      operands = _arg.operands;
      operands = operands.map(makeApplyFn);
      return function(ds) {
        return operands[0](ds) * operands[1](ds);
      };
    },
    divide: function(_arg) {
      var operands;
      operands = _arg.operands;
      operands = operands.map(makeApplyFn);
      return function(ds) {
        return operands[0](ds) / operands[1](ds);
      };
    }
  };

  makeApplyFn = function(cmd) {
    var applyFn;
    applyFn = applyFns[cmd.aggregate];
    if (!applyFn) {
      throw new Error("No such aggregate `" + cmd.aggregate + "` in apply");
    }
    return applyFn(cmd);
  };

  compareFns = {
    ascending: function(a, b) {
      if (a < b) {
        return -1;
      } else if (a > b) {
        return 1;
      } else if (a >= b) {
        return 0;
      } else {
        return NaN;
      }
    },
    descending: function(a, b) {
      if (b < a) {
        return -1;
      } else if (b > a) {
        return 1;
      } else if (b >= a) {
        return 0;
      } else {
        return NaN;
      }
    }
  };

  sortFns = {
    natural: function(_arg) {
      var compareFn, direction, prop;
      prop = _arg.prop, direction = _arg.direction;
      compareFn = compareFns[direction];
      if (!compareFn) {
        throw "direction has to be 'ascending' or 'descending'";
      }
      return function(a, b) {
        return compareFn(a.prop[prop], b.prop[prop]);
      };
    },
    caseInsensetive: function(_arg) {
      var compareFn, direction, prop;
      prop = _arg.prop, direction = _arg.direction;
      compareFn = compareFns[direction];
      if (!compareFn) {
        throw "direction has to be 'ascending' or 'descending'";
      }
      return function(a, b) {
        return compareFn(String(a.prop[prop]).toLowerCase(), String(b.prop[prop]).toLowerCase());
      };
    }
  };

  computeQuery = function(data, query) {
    var applyFn, cmd, compareFn, filterFn, limit, propName, rootSegment, segment, segmentGroup, segmentGroups, sortFn, splitFn, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _m, _n, _o, _p, _q;
    if (!query) {
      throw new Error("query not given");
    }
    rootSegment = {
      _raw: data,
      prop: {}
    };
    segmentGroups = [[rootSegment]];
    for (_i = 0, _len = query.length; _i < _len; _i++) {
      cmd = query[_i];
      switch (cmd.operation) {
        case 'filter':
          filterFn = makeFilterFn(cmd);
          for (_j = 0, _len1 = segmentGroups.length; _j < _len1; _j++) {
            segmentGroup = segmentGroups[_j];
            for (_k = 0, _len2 = segmentGroup.length; _k < _len2; _k++) {
              segment = segmentGroup[_k];
              segment._raw = segment._raw.filter(filterFn);
            }
          }
          break;
        case 'split':
          propName = cmd.name;
          if (!propName) {
            throw new Error("'name' not defined in split");
          }
          splitFn = makeSplitFn(cmd);
          segmentGroups = driverUtil.flatten(segmentGroups).map(function(segment) {
            var bucketValue, buckets, d, key, keyString, keys, _l, _len3, _ref;
            keys = [];
            buckets = {};
            bucketValue = {};
            _ref = segment._raw;
            for (_l = 0, _len3 = _ref.length; _l < _len3; _l++) {
              d = _ref[_l];
              key = splitFn(d);
              if (key == null) {
                throw new Error("Bucket returned undefined");
              }
              keyString = String(key);
              if (!buckets[keyString]) {
                keys.push(keyString);
                buckets[keyString] = [];
                bucketValue[keyString] = key;
              }
              buckets[keyString].push(d);
            }
            segment.splits = keys.map(function(keyString) {
              var prop;
              prop = {};
              prop[propName] = bucketValue[keyString];
              return {
                _raw: buckets[keyString],
                prop: prop
              };
            });
            driverUtil.cleanSegment(segment);
            return segment.splits;
          });
          break;
        case 'apply':
          propName = cmd.name;
          if (!propName) {
            throw new Error("'name' not defined in apply");
          }
          applyFn = makeApplyFn(cmd);
          for (_l = 0, _len3 = segmentGroups.length; _l < _len3; _l++) {
            segmentGroup = segmentGroups[_l];
            for (_m = 0, _len4 = segmentGroup.length; _m < _len4; _m++) {
              segment = segmentGroup[_m];
              segment.prop[propName] = applyFn(segment._raw);
            }
          }
          break;
        case 'combine':
          if (cmd.sort) {
            for (_n = 0, _len5 = segmentGroups.length; _n < _len5; _n++) {
              segmentGroup = segmentGroups[_n];
              sortFn = sortFns[cmd.sort.compare];
              if (!sortFn) {
                throw new Error("No such compare `" + cmd.sort.compare + "` in combine.sort");
              }
              compareFn = sortFn(cmd.sort);
              for (_o = 0, _len6 = segmentGroups.length; _o < _len6; _o++) {
                segmentGroup = segmentGroups[_o];
                segmentGroup.sort(compareFn);
              }
            }
          }
          if (cmd.limit != null) {
            limit = cmd.limit;
            for (_p = 0, _len7 = segmentGroups.length; _p < _len7; _p++) {
              segmentGroup = segmentGroups[_p];
              segmentGroup.splice(limit, segmentGroup.length - limit);
            }
          }
          break;
        default:
          throw new Error("Unknown operation '" + cmd.operation + "'");
      }
    }
    for (_q = 0, _len8 = segmentGroups.length; _q < _len8; _q++) {
      segmentGroup = segmentGroups[_q];
      segmentGroup.forEach(driverUtil.cleanSegment);
    }
    return rootSegment;
  };

  exports = function(data) {
    return function(query, callback) {
      var result;
      try {
        result = computeQuery(data, query);
      } catch (e) {
        callback({
          message: e.message,
          stack: e.stack
        });
        return;
      }
      callback(null, result);
    };
  };

  if (typeof module === 'undefined') {
    window['simpleDriver'] = exports;
  } else {
    module.exports = exports;
  }

}).call(this);
