...
{ FacetApply, ApplySimplifier } = facet.legacy
...
describe "ApplySimplifier", ->
  ...
  describe "single dataset", ->
    it "works on basic example (leave constant)", ->
      ...
      applySimplifier.addApplies(applySpecs.map(fromJS))

      expect(applySimplifier.getSimpleApplies().map(getJS)).to.deep.equal(
        applySpecs
      )
  ...
  describe "multi dataset", ->
    it "works in a single-dataset case", ->
      ...
      expect(appliesByDataset.main.map(getJS)).to.deep.equal([
        {
          name: 'Count', aggregate: 'sum', attribute: 'count'
        }
      ])
    ...
    it "works in a simple multi-dataset case and opts for the simple name", ->
      ...
      expect(appliesByDataset).to.be.an('object')
      expect(appliesByDataset.humans.map(getJS)).to.deep.equal([
        {
          name: 'HumanCount', dataset: 'humans', aggregate: 'sum', attribute: 'count'
        }
      ])
      expect(appliesByDataset.robots.map(getJS)).to.deep.equal([
        {
          name: 'RobotCount', dataset: 'robots', aggregate: 'sum', attribute: 'count'
        }
      ])
    ...
    it "works with multi dataset averages", ->
      ...
      expect(appliesByDataset).to.be.an('object')
      expect(appliesByDataset['ideal-cut'].map(getJS)).to.deep.equal([
        {
      ...
      expect(appliesByDataset['good-cut'].map(getJS)).to.deep.equal([
        {
    ...
    it "works in the case of a delta", ->
      ...
      expect(applySimplifier.getApplyComponents("count_delta_").map((apply) -> apply.toJS())).to.deep.equal([
        {
          "name": "count",
          "aggregate": "sum",
          "attribute": "count"
      ...
      expect(appliesByDataset.prev.map(getJS)).to.deep.equal([
        {
          dataset: "prev"
          name: "_S2_count_delta_"
          aggregate: "sum"
