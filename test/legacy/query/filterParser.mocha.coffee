...
describe "parser", ->
  describe "Errors", ->
    it "should error on unclosed quotes", ->
      ...
      expect(-> FacetFilter.parse(qry)).to.throw(Error, "Expected Value but \"\\\"\" found.")

    it "should error on unclosed parentheses", ->
      qry = "country is france or (color is yellow and color is red or color is purple"
      expect(-> FacetFilter.parse(qry)).to.throw(Error, "Expected \")\", \"and\" or \"or\" but end of input found.")

    it "should error on malformed input", ->
      qry = "country is france color is yellow and color is red or color is purple"
      expect(-> FacetFilter.parse(qry)).to.throw(Error, "Expected \"and\", \"or\" or end of input but \"c\" found.")
      qry = "country is color is yellow and color is red or color is purple"
      expect(-> FacetFilter.parse(qry)).to.throw(Error, "Expected \"and\", \"or\" or end of input but \"i\" found.")


  describe "Successful parsing", ->
    it "can deal with a combination of ands and ors correctly - case: 1", ->
      qry = "country is france or color is yellow and color is red or color is purple"
      expect(FacetFilter.parse(qry).toJS()).to.deep.equal({
        "type": "and"
        "filters": [
          {
            "type": "or"
